...
# Loads GRIB2 Data
ds = xr.load_dataset('ds.maxt.bin', engine='cfgrib')

grbs = pygrib.open('ds.maxt.bin')
...
if count == 2:
    ...
    cs = ax.contourf(lons, lats, grb_24_F, levels=np.arange(50, 140, 10), cmap='coolwarm', transform=datacrs)
    cbar_RH = fig.colorbar(cs, shrink=0.80)
    cbar_RH.set_label(label="Max T \N{DEGREE SIGN}F", fontweight='bold')
    #---------------------------------------------------------------------------------------------------------------------
    # Day 2 Trend
    #--------------------------------------------------------------------------------------------------------------------------------
    ...
    cbar_RH1.set_label(label="Max T \N{DEGREE SIGN}F", fontweight='bold')
...
if count == 3:
    ...
    cbar_RH2.set_label(label="Max T \N{DEGREE SIGN}F", fontweight='bold')

plt.savefig("NWS_max_T_Forecast")
fp.close()
