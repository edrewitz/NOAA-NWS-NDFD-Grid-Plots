...
if count == 2:
    ...
    cs = ax.contourf(lons, lats, grb_12.values, levels=np.arange(0, 16, 1), cmap='YlOrBr_r', transform=datacrs)
    ...
    cs1 = ax1.contourf(lons, lats, grb_24.values, levels=np.arange(0, 16, 1), cmap='YlOrBr_r', transform=datacrs)
...
if count == 3:
    ...
    cs2 = ax2.contourf(lons, lats, grb_48.values, levels=np.arange(0, 16, 1), cmap='YlOrBr_r', transform=datacrs)
    cbar_RH2 = fig.colorbar(cs2, shrink=0.80)
    cbar_RH2.set_label(label="Min RH Trend (%)", fontweight='bold')

plt.savefig(f"Weather Data/NWS Filtered Min RH")
fp.close()
