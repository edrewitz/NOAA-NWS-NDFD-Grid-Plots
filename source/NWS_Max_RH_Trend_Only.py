...
ds = xr.load_dataset('ds.maxrh.bin', engine='cfgrib')

grbs = pygrib.open('ds.maxrh.bin')
...
if count == 2:
    ...
    # Day 2 Trend
    #--------------------------------------------------------------------------------------------------------------------------------
    ax1 = plt.subplot(1, 1, 1, projection=mapcrs)
...
if count == 3:
    if night_2.day == grb_24.validDate.day:
        #---------------------------------------------------------------------------------------------------------------------
        # Day 2 Trend
        #--------------------------------------------------------------------------------------------------------------------------------
        ax1 = plt.subplot(1, 2, 1, projection=mapcrs)
        ax1.set_extent([-122, -114, 31, 39], datacrs)
        ax1.add_feature(cfeature.COASTLINE.with_scale('50m'), linewidth=0.75)
        ax1.add_feature(cfeature.STATES, linewidth=0.5)
        ax1.add_feature(USCOUNTIES, linewidth=0.75)
        ax1.set_title('Maximum RH Forecast Trend (Tonight)\nStart: '+ grib2s_t.strftime('%m/%d/%Y %HZ') + '\nEnd:'+ grib2e_t.strftime('%m/%d/%Y %HZ'), fontsize=10, fontweight='bold', loc='center')
        ...
        # Day 3 Trend
        #--------------------------------------------------------------------------------------------------------------------------------
        ax2 = plt.subplot(1, 2, 2, projection=mapcrs)
    ...
    if night_2.day == grb_48.validDate.day:
        #---------------------------------------------------------------------------------------------------------------------
        # Day 2 Trend
        #--------------------------------------------------------------------------------------------------------------------------------
        ax1 = plt.subplot(1, 2, 1, projection=mapcrs)
        ...
        ax1.add_feature(cfeature.STATES, linewidth=0.5)
        ax1.add_feature(USCOUNTIES, linewidth=0.75)
        ax1.set_title('Maximum RH Forecast Trend (Tomorrow Night)\nStart: '+ grib2s_t.strftime('%m/%d/%Y %HZ') + '\nEnd:'+ grib2e_t.strftime('%m/%d/%Y %HZ'), fontsize=10, fontweight='bold', loc='center')
...
plt.savefig(f"Weather Data/NWS Max RH TREND ONLY")
