...
# Opens file 
with open('ds.dryfireo.bin', 'wb') as fp:
    ftp.retrbinary('RETR ds.dryfireo.bin', fp.write)
ftp.close()

# Loads GRIB2 Data
ds = xr.load_dataset('ds.dryfireo.bin', engine='cfgrib')

grbs = pygrib.open('ds.dryfireo.bin')
...
fig.text(0.13, 0.05, 'Developed by: Eric Drewitz - Powered by MetPy | Data Source: NOAA/NWS/NDFD\nImage Created: ' + date1.strftime('%m/%d/%Y %H:%MZ'), fontweight='bold')
fig.text(0.76, 0.90, 'Key:\nIsolated - Yellow\nScattered - Red', fontsize=8, fontweight='bold')
...
if grb_12_logic == True:
    ...
    ax.set_title('Dry Lightning Thunderstorm Risk Forecast (Day 1)\nStart: '+ grib1s_t.strftime('%m/%d/%Y %HZ') + '\nEnd:'+ grib1e_t.strftime('%m/%d/%Y %HZ'), fontsize=10, fontweight='bold', loc='center')
        
    cs = ax.contourf(lons, lats, prob_12, levels=np.arange(4, 10, 2), cmap='YlOrRd', transform=datacrs)
    
    fig.savefig("Dry_Lightning_Day_1_Forecast")
    fp.close()
...
fig1.text(0.13, 0.05, 'Developed by: Eric Drewitz - Powered by MetPy | Data Source: NOAA/NWS/NDFD\nImage Created: ' + date1.strftime('%m/%d/%Y %H:%MZ'), fontweight='bold')
fig1.text(0.76, 0.90, 'Key:\nIsolated - Yellow\nScattered - Red', fontsize=8, fontweight='bold')
...
if grb_24_logic == True:
    ...
    ax1.set_title('Dry Lightning Thunderstorm Risk Forecast (Day 2)\nStart: '+ grib2s_t.strftime('%m/%d/%Y %HZ') + '\nEnd:'+ grib2e_t.strftime('%m/%d/%Y %HZ'), fontsize=10, fontweight='bold', loc='center')
        
    cs1 = ax1.contourf(lons, lats, prob_24, levels=np.arange(4, 10, 2), cmap='YlOrRd', transform=datacrs)
    
    fig1.savefig("Dry_Lightning_Day_2_Forecast")
    fp.close()
...
fig2.text(0.13, 0.05, 'Developed by: Eric Drewitz - Powered by MetPy | Data Source: NOAA/NWS/NDFD\nImage Created: ' + date1.strftime('%m/%d/%Y %H:%MZ'), fontweight='bold')
fig2.text(0.76, 0.90, 'Key:\nIsolated - Yellow\nScattered - Red', fontsize=8, fontweight='bold')
...
if grb_48_logic == True:
    ...
    ax2.set_title('Dry Lightning Thunderstorm Risk Forecast (Day 3)\nStart: '+ grib3s_t.strftime('%m/%d/%Y %HZ') + '\nEnd:'+ grib3e_t.strftime('%m/%d/%Y %HZ'), fontsize=10, fontweight='bold', loc='center')
        
    cs2 = ax2.contourf(lons, lats, prob_48, levels=np.arange(4, 10, 2), cmap='YlOrRd', transform=datacrs)
    
    fig2.savefig("Dry_Lightning_Day_3_Forecast")
    fp.close()
